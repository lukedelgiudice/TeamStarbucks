...
precompute_yards_distributions_unexpected <- function() {
  play_data <- readRDS("unexpected_plays_data.rds") %>%
    mutate(
      play_call = play_type,
  ...
  saveRDS(distributions, "yards_distributions_unexpected.rds")
  return(distributions)
}

distributions_unexpected <- precompute_yards_distributions_unexpected()